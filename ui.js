...
window.renderGitHubCard = function(gh) {
  if (!gh || !gh.connected) {
    return '<div class="error-msg">' + window.escHtml(gh && gh.error ? gh.error : 'Not connected') + '</div>';
  }
  return '<div class="stat-row"><span class="stat-label">Repository</span><span class="stat-value mono">' + window.escHtml(gh.repo) + '</span></div>' +
    '<div class="stat-row"><span class="stat-label">Branch</span><span class="stat-value">' + window.escHtml(gh.branch) + '</span></div>' +
...
window.renderVercelCard = function(vc) {
  if (!vc || !vc.connected) {
    return '<div class="error-msg">' + window.escHtml(vc && vc.error ? vc.error : 'Not connected') + '</div>';
  }
  var statusText = window.escHtml(vc.status || 'unknown');
  return '<div class="stat-row"><span class="stat-label">Status</span><span class="stat-value">' + statusText + '</span></div>' +
    ...
    '<div class="stat-row"><span class="stat-label">Commit</span><span class="stat-value mono">' + window.escHtml(vc.commitSha || '-') + '</span></div>' +
    (vc.commitMessage ? '<div class="stat-row"><span class="stat-label">Message</span><span class="stat-value">' + window.escHtml(vc.commitMessage.slice(0, 40)) + '</span></div>' : '');
};

window.renderSupabaseCard = function(sb) {
  if (!sb || !sb.connected) {
    return '<div class="error-msg">' + window.escHtml(sb && sb.error ? sb.error : 'Not connected') + '</div>';
  }
...
window.renderCommitsList = function(commits) {
  ...
  return commits.map(function(c) {
    return '<div class="commit-item">' +
      '<div class="commit-msg">' + window.escHtml(c.message || '') + '</div>' +
      '<div class="commit-meta"><span class="commit-sha">' + window.escHtml(c.sha || '') + '</span> by ' + window.escHtml(c.author || '') + ' — ' + window.timeAgo(c.date) + '</div>' +
    '</div>';
  }).join('');
};